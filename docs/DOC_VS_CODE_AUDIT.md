...
## 7. Исправления (выполнено 19.02.2026)

| # | Что исправлено | Файл(ы) |
|---|---|---|
| 1 | Переписан устаревший REPO_STATUS_REPORT (v1→v2) | `REPO_STATUS_REPORT.md` |
| 2 | Тег `karaoke` → `vip` | `DATABASE_SCHEMA.md` |
| 3 | Иконка `fork-knife` → `utensils` | `DATABASE_SCHEMA.md`, `API_SPEC.md` |
| 4 | "50 заведений" → "100 заведений" (Gemini контекст) | `MVP_SPEC.md` |
| 5 | Убран `bounds` из heatmap response | `API_SPEC.md` |
| 6 | Уточнён heatmap default resolution (low, precision 2) | `API_SPEC.md` |
| 7 | "12 компонентов" → "14 компонентов" | `ARCHITECTURE.md`, `ROADMAP.md`, `README.md` |
| 8 | Добавлена документация `scripts/start.mjs` | `README.md`, `ARCHITECTURE.md` |

### Оставшиеся рекомендации (приоритет 3)
1. Добавить CHANGELOG.md
2. Добавить Swagger/OpenAPI spec (автогенерация из route handlers)
3. Добавить ADR-002 для решений, принятых во время разработки (отказ от Prisma 7, etc.)

---

*Аудит проведён 19.02.2026. Исправления применены 19.02.2026.*
